...
namespace Vodovoz.ViewModels.Journals.Filters.Orders
{
	public class OrderJournalFilterViewModel : FilterViewModelBase<OrderJournalFilterViewModel>
	{
			...
			params Action<OrderJournalFilterViewModel>[] filterParams)
		{
			...
			{
				throw new ArgumentNullException(nameof(currentUserSettings));
			}
			...
			DeliveryPointSelectorFactory = _deliveryPointJournalFactory.CreateDeliveryPointByClientAutocompleteSelectorFactory(scope);
		...
		public virtual Counterparty RestrictCounterparty
		{
			...
			set
			{
				if(UpdateFilterField(ref _restrictCounterparty, value))
				{
					...
					//_deliveryPointJournalFactory.Counterparty = value;
					if(value == null)
					{
						_deliveryPointJournalFactory.SetDeliveryPointJournalFilterViewModel(null);
						DeliveryPoint = null;
